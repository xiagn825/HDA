	...
	$("#queryschoolad").children("tbody").children("tr").each(function(iTR){
		if (iTR >= 2 && $(this).attr("class") != "getJsXmlTr") {
			...
			if (record != "") {
				...
				for (var iCategories in collegeCategories) {
					categoryCode = collegeCategories[iCategories].code;
					categoryName = collegeCategories[iCategories].name;
					
					if (decodedTabUrl.indexOf("schoolprop=" + categoryName) > -1) {
...
function reader() {
	...
	chrome.extension.sendRequest({}, function(response){
		...
		if (decodedTabUrl == "http://www.eol.cn/") {
			url = "http://gkcx.eol.cn/soudaxue/queryschool.html?schoolprop=" + collegeCategories[0].name;
				...
				if ($(this).text() == "下一页") {
					if ($(this).attr("class") == "pl") {
						for (var iCategories in collegeCategories) {
							...
							if (decodedTabUrl.indexOf("schoolprop=" + categoryName) > -1) {
								url = "http://gkcx.eol.cn/soudaxue/queryschool.html?schoolprop=" + categoryName + "&page=" + (currentPage + 1);
								window.location.href = encodeURI(url);
							}
						}
					}
						...
						for (var iCategories in collegeCategories) {
							...
							if (decodedTabUrl.indexOf("schoolprop=" + categoryName) > -1) {
								if (collegeCategories.length > (parseInt(iCategories) + 1)) {
									url = "http://gkcx.eol.cn/soudaxue/queryschool.html?schoolprop=" + collegeCategories[parseInt(iCategories) + 1].name;
									window.location.href = encodeURI(url);
								}
								else {
