...
function reader() {
	...
	chrome.extension.sendRequest({}, function(response){
		...
		if (url == "http://www.eol.cn/") {
			url = "http://gkcx.eol.cn/schoolhtm/schoolAreaPoint/" + scoreCollegeDetailColleges[0] + "/" + scoreCollegeDetailProvinces[0] + "/" + scoreCollegeDetailStudentCategories[0] + "/" + scoreCollegeDetailBatches[0] + ".htm";
			...
			if (!hasSetUrl) {
				var iCollege
				for (iCollege in scoreCollegeDetailColleges) {
					if (college == scoreCollegeDetailColleges[iCollege]) {
						if (iCollege < scoreCollegeDetailColleges.length - 1) {
							url = "http://gkcx.eol.cn/schoolhtm/schoolAreaPoint/" + scoreCollegeDetailColleges[parseInt(iCollege) + 1] + "/" + scoreCollegeDetailProvinces[0] + "/" + scoreCollegeDetailStudentCategories[0] + "/" + scoreCollegeDetailBatches[0] + ".htm";
							window.location.href = url;
							hasSetUrl = true;
						}
					}
