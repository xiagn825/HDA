function writer(decodedTabUrl) {
	...
	$("#queryschoolad").children("tbody").children("tr").each(function(iTR){
		if (iTR >= 2) {
			...
			if (record != "") {
				window.localStorage.setItem(specialtyCategories[24].name + "_" + formatNumberByZero(page, 3) + "_" + formatNumberByZero(lineNum, 2), record);
			}
		}
	});
...
function reader() {
	...
	chrome.extension.sendRequest({}, function(response){
		...
		if (decodedTabUrl == "http://www.eol.cn/") {
			url = "http://gkcx.eol.cn/soudaxue/querySchoolSpecialty.html?zkzytype=" + specialtyCategories[24].name;
			...
			$("#pageid").children("a").each(function(idx){
				...
				if ($(this).text() == "下一页") {
					if ($(this).attr("class") == "pl") {
						url = "http://gkcx.eol.cn/soudaxue/querySchoolSpecialty.html?zkzytype=" + specialtyCategories[24].name + "&page=" + (currentPage + 1);
						window.location.href = encodeURI(url);
					}
				}
			});
